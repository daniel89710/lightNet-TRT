...
void write_prediction(
  std::string dumpPath, std::string filename, std::vector<std::string> names,
  std::vector<Result> objects, int width, int height)
{
  ...
  for (const auto & object : objects) {
    ...
    std::string writing_text =
      format("%s %f %d %d %d %d", names[object.id].c_str(), object.prob, left, top, right, bottom);
...
int main(int argc, char ** argv)
{
  ...
  config.file_model_cfg =
    yoloInfo
      .configFilePath;  //"../configs/yolov7-tiny-relu-BDD100K-960x960-opt-params-mse-sparse.cfg";
  config.file_model_weights =
    yoloInfo
      .wtsFilePath;  //"../configs/yolov7-tiny-relu-BDD100K-960x960-opt-params-mse-sparse_last.weights";
  ...
  config1.file_model_weights =
  yoloInfo1.wtsFilePath;//"../configs/yolov7-tiny-relu-BDD100K-960x960-opt-params-mse-sparse_last.weights";
