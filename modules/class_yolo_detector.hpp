...
public:
  ...
  void draw_BBox(cv::Mat & img, Result result)
  {
    ...
    if (colormap) {
      cv::rectangle(
        img, result.rect,
        cv::Scalar(colormap[3 * id + 2], colormap[3 * id + 1], colormap[3 * id + 0]), 4);
    } else {
    ...
    if (colormap) {
      cv::putText(
        img, stream.str(), cv::Point(result.rect.x, result.rect.y - 5), 0, 0.5,
        cv::Scalar(colormap[3 * id + 2], colormap[3 * id + 1], colormap[3 * id + 0]), 1);
    } else {
      cv::putText(
        img, stream.str(), cv::Point(result.rect.x, result.rect.y - 5), 0, 0.5,
        cv::Scalar(255, 0, 0), 1);
  ...
  void dump_profiling() { _p_net->print_profiling(); }

  void detect(
    const std::vector<cv::Mat> & vec_image, std::vector<BatchResult> & vec_batch_result,
    const bool cuda)
  {
    cv::Mat trtInput;
    ...
    for (uint32_t i = 0; i < vec_ds_images.size(); ++i) {
      ...
      // auto segmentation = _p_net->apply_argmax(i, curImage.getImageHeight(),
      // curImage.getImageWidth());
      auto remaining = nmsAllClasses(
        _p_net->getNMSThresh(), binfo, _p_net->getNumClasses(), _vec_net_type[_config.net_type]);
      if (remaining.empty()) {
        continue;
      }
