...
cudaError_t cuda_hardswish_layer(
  const void * input_, void * output_, const int n_batch_size_, const int n_output_size_,
  const int threads_, cudaStream_t stream_)
{
  ...
  kernel_hardswish<<<(n_data_size + threads_ - 1) / threads_, threads_, 0, stream_>>>(
    reinterpret_cast<const float *>(input_), reinterpret_cast<float *>(output_), n_data_size);
  return cudaGetLastError();
}

int Hardswish::enqueue(
  int batchSize, const void * const * inputs, void * const * outputs, void * workspace,
  cudaStream_t stream) noexcept
{
  //		printf("batch_size:%d,output_size:%d,threads:%d\n", batchSize, _n_output_size,
  //_n_max_thread_pre_block);
  NV_CUDA_CHECK(cuda_hardswish_layer(
    inputs[0], outputs[0], batchSize, _n_output_size, _n_max_thread_pre_block, stream));
  return 0;
}
...
void Hardswish::configureWithFormat(
...
void Hardswish::configurePlugin(
  const PluginTensorDesc * in, int nbInput, const PluginTensorDesc * out, int nbOutput) noexcept
{
