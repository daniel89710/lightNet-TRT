...
__global__ void gpuYoloV4Layer(const float* input, float* output, const uint32_t grid_h_,
								const uint32_t grid_w_, const uint32_t numOutputClasses,
                               const uint32_t numBBoxes, float scale_x_y)
{
    ...
    scale_x_y = 1.0;
    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 0)]
      = scale_x_y * (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 0)]) - 0.5 * (scale_x_y -1);
    //by
    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 1)]
        = scale_x_y * (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 1)]) - 0.5 * (scale_x_y -1);
    //bw
    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 2)]
        = (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 2)]) * (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 2)]) * 4;
    //bh
    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 3)]
        = (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 3)]) * (input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 3)]) * 4;
