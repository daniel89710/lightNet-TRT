...
Detect::Detect(const void * data, size_t length)
{
  ...
  // printf("anchor:%d,classes:%d,gh:%d,gw:%d,size:%d\n", _n_anchor, _n_classes, _n_grid_h,
  // _n_grid_w, _n_output_size);
  assert(d == a + length);
}
...
__global__ void gpu_detect_layer(
  const float * input_, float * output_, const uint32_t n_grid_h_, const uint32_t n_grid_w_,
  const uint32_t n_classes_, const uint32_t n_anchor_)
{
...
cudaError_t cuda_detect_layer(
  const void * input_, void * output_, const uint32_t & batch_size_, const uint32_t & grid_h_,
  const uint32_t & grid_w_, const uint32_t & n_classes_, const uint32_t & n_anchor_,
  uint64_t n_output_size_, cudaStream_t stream_)
{
  dim3 threads_per_block(16, 16, 4);
  dim3 number_of_blocks(
    (grid_w_ / threads_per_block.x) + 1, (grid_h_ / threads_per_block.y) + 1,
  ...
  for (int batch = 0; batch < batch_size_; ++batch) {
    gpu_detect_layer<<<number_of_blocks, threads_per_block, 0, stream_>>>(
      ...
      reinterpret_cast<float *>(output_) + (batch * n_output_size_), grid_h_, grid_w_, n_classes_,
      n_anchor_);
  }
...
{
  NV_CUDA_CHECK(cuda_detect_layer(
    inputs[0], outputs[0], batchSize, _n_grid_h, _n_grid_w, _n_classes, _n_anchor, _n_output_size,
    stream));
  return 0;
}
...
void Detect::configureWithFormat(
