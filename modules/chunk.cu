...
namespace nvinfer1
{
		...
		cudaStream_t stream) noexcept
	{
		...
		{
		printf("Chunk %d, %d\n", _n_size_split, _n_size_split/sizeof(float));
		/*
			NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[0] + b * _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice, stream));
			NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[1] + b * _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice, stream));
			*/
			NV_CUDA_CHECK(cudaMemcpy((char*)outputs[0] + b * _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice));
			NV_CUDA_CHECK(cudaMemcpy((char*)outputs[1] + b * _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice));			
		}
	...
	void Chunk::configurePlugin(const PluginTensorDesc* in, int nbInput, const PluginTensorDesc* out, int nbOutput)
	{
		_n_size_split = in->dims.d[0] / 2 * in->dims.d[1] * in->dims.d[2] *sizeof(float);
				printf("Chunk in configuring %d, %d\n", _n_size_split, _n_size_split/sizeof(float));
	}
