  ...
  cudaStream_t stream) noexcept
{
  ...
  for (int b = 0; b < batchSize; ++b) {
    ...
    /*
            NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[0] + b * _n_size_split, (char*)inputs[0] +
       b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice,
       stream)); NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[1] + b * _n_size_split,
       (char*)inputs[0] + b * 2 * _n_size_split + _n_size_split/sizeof(float), _n_size_split,
       cudaMemcpyDeviceToDevice, stream));
            */
    NV_CUDA_CHECK(cudaMemcpy(
      (char *)outputs[0] + b * _n_size_split,
      (char *)inputs[0] + b * 2 * _n_size_split + _n_size_split / sizeof(float), _n_size_split,
  ...
  //	NV_CUDA_CHECK(cudaMemcpy(outputs[1], (void*)((char*)inputs[0] + _n_size_split),
...
DataType Chunk::getOutputDataType(
...
bool Chunk::isOutputBroadcastAcrossBatch(
...
void Chunk::attachToContext(
  cudnnContext * cudnnContext, cublasContext * cublasContext, IGpuAllocator * gpuAllocator)
{
}

void Chunk::configurePlugin(
...
void Chunk::configureWithFormat(
...
IPluginV2 * ChunkPluginCreator::deserializePlugin(
