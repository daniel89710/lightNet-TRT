	...
	for (uint32_t y = 0; y < tensor.grid_h; ++y)
	{
		for (uint32_t x = 0; x < tensor.grid_w; ++x)
		{
			for (uint32_t b = 0; b < tensor.numBBoxes; ++b)
			{
				...
				const float bx
					= x + scale_x_y * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 0)] - offset;
				const float by
					= y + scale_x_y * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 1)] - offset;
				const float bw
					= pw * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 2)]  * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 2)] * 4;
				const float bh
					= ph * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 3)] * detections[bbindex + numGridCells * (b * (5 + tensor.numClasses) + 3)] * 4;
